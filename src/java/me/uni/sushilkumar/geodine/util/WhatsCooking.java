...
public class WhatsCooking extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            URL u = new URL("http://www.food.com/rss");
            JSONObject obj = new JSONObject();
            JSONArray array = new JSONArray();
            Document doc = Jsoup.parse(u.openStream(), "UTF-8", "", Parser.xmlParser());
            Elements item = doc.getElementsByTag("item");

            for (int i = 0; i < 5; i++) {
                JSONObject temp = new JSONObject();
                String title = item.get(i).children().get(0).text();
                String link = item.get(i).children().get(1).text();
